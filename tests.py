...
def test_unfoldings():
    ...
    A_unfold_1 = unfold(A, 1)
    A_unfold_2 = unfold(A, 2)
    A_unfold_3 = unfold(A, 3)
    max_i = A.shape[0]
    max_j = A.shape[1]
    max_k = A.shape[2]
    ...
    for i in range(A.shape[0]):
        for j in range(A.shape[1]):
            for k in range(A.shape[2]):
                check_unfold_1.append(A[i, j, k] == A_unfold_1[i, j + (k * max_j)])
                # check_unfold_2.append(A[i, j, k] == A_unfold_2[j, i + (k * max_i)])
                check_unfold_3.append(A[i, j, k] == A_unfold_3[k, i + (j * max_i)])
...
def test_H_accumulators():
    ...
    X_train, Y_train = generate_dense_dataset(100, D, I, scale=0.1, noise_variance=0)
    model = BTTKM(D, ranks, dims, pure_power_features_full)
    model.train(X_train, Y_train, iteration_limit=0)
...
def test_H_forward_against_G():
    """ set covariance_WW in H update to 0 """
    D = 3  # Number of cores
    I = 2
    ranks = [3 for _ in range(D - 1)]  # Tensor-train ranks
    ranks = [1] + ranks + [1]  # first and last rank must be 1 to maintain output dimension
    ...
    d = model.D - 1
    G_lt = khatri_rao(model.feature_map[d], model.forward_accumulator_G(d))  # N x R_d M_d
    G_d = khatri_rao(model.backward_accumulator_G(d), G_lt)  # N x R_d M_d R_{d+1}
    GTG = G_d.T @ G_d
    ...
    H_d_tens = H_d_mat.reshape([model.M[d], model.R[d], model.R[d], model.R[d + 1], model.R[d + 1], model.M[d]], order='F')
    ...
    H_d = H_d_tens_trans.reshape([model.R[d] * model.M[d] * model.R[d + 1], model.R[d] * model.M[d] * model.R[d + 1]],
                                 order='F')
...
def forward_step_by_step():
    ...
    model.train(X_train, Y_train, iteration_limit=0)
    model.W.cores[0] = np.array([[[1,0],[0,1], [1,0]]])
    model.W.cores[1] = np.array([[[2,0],[0,2],[2,0]],[[0, 2], [2,0],[0,2]]])
    model.W.cores[2] = np.array([[[-1],[0],[-1]],[[0],[-1],[0]]])
...
def backward_step_by_step():
    ...
    model = BTTKM(D, ranks, dims, no_kernel)
    model.train(X_train, Y_train, iteration_limit=0)
    model.W.cores[0] = 0.1*np.array([[[1,3, 3],[2,4,4]]])
    model.W.cores[1] = 0.1*np.array([[[1,7,13],
                                           [4,10,16]],
                                          ...
                                          [[3,9,15],
                                           [6,12,18]]])
    model.W.cores[2] = 0.1*np.array([[[13],[15]],[[14],[16]],[[17],[18]]])
