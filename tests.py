...
def test_H_forward_against_G():
    ...
    d = model.D - 1
    G_lt = khatri_rao(model.forward_accumulator_G(d), model.feature_map[d])  # N x M_d R_{d+1}
    G = khatri_rao(G_lt, model.backward_accumulator_G(d))  # N x R_d M_d R_{d+1}
    GTG = G.T @ G

    H_lt = khatri_rao(model.forward_accumulator_H(d), model.feature_map[d])  # N x M_d R_{d+1} R_d{+1}
    H_gt = khatri_rao(model.feature_map[d], model.backward_accumulator_H(d))  # N x R_d R_d M_d
    H_d = H_lt.T @ H_gt  # M_d R_{d+1} R_{d+1} R_d R_d M_d
    H_d = H_d.reshape([model.M[d], model.R[d + 1], model.R[d + 1], model.R[d], model.R[d], model.M[d]], order='F')
    H_d = H_d.transpose([3, 0, 1, 4, 5, 2])
...
def forward_step_by_step():
    ...
    model = BTTKM(D, ranks, dims, no_kernel)
    model.train(X_train, Y_train, iteration_limit=0)
    model.W.cores[0] = Core(np.array([[[1,0,2],[0,1,0]]]))
    model.W.cores[1] = Core(np.array([[[1,0,4],
                                        [0,2,0]],
                                       ...
                                       [[2,0,5],
                                        [0,3,0]]]))
    model.W.cores[2] = Core(np.array([[[-1],[0]],[[0],[-1]], [[-1],[0]]]))

    model.feature_map[0] = np.eye(2)
    model.feature_map[1] = np.eye(2)
    model.feature_map[2] = np.eye(2)
...
def backward_step_by_step():
    ...
    model.train(X_train, Y_train, iteration_limit=0)
    model.W.cores[0] = Core(np.array([[[1, 0,2],[0,1,0]]]))
    model.W.cores[1] = Core(np.array([[[1,7,13],
                                           [4,10,16]],
                                          ...
                                          [[3,9,15],
                                           [6,12,18]]]))
    model.W.cores[2] = Core(np.array([[[13],[15]],[[14],[16]],[[17],[18]]]))
