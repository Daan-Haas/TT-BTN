    ...
    def train(self,
        ...
        for it in pbar:
            ...
            for d in range(self.D):
                ...
                lambda_term = np.sum((1-np.log(self.d_N[d]) - (d_0[d]/self.d_N[d]))*self.c_N[d] + np.log(gamc))
                delta_term = np.sum((1-np.log(self.h_N[d]) - (h_0[d]/self.h_N[d]))*self.g_N[d] + np.log(gamg))
